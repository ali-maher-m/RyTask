...
/**
 * A fuller meta description than the bare tool summary: it reads as a standalone
 * search/AI snippet (keywords: RyTask, MCP tool, AI agents) and names the
 * permission the tool needs. Flows into the page title, OG image, and the
 * TechArticle JSON-LD the docs page emits.
 */
function toolMetaDescription(tool, permission) {
  const base = String(tool.description).trim().replace(/\.$/, '');
  return `${base} — a RyTask MCP tool for AI agents over the Model Context Protocol (requires the ${permission} permission).`;
}
...
function toolPage(tool, permissions) {
  ...
  const lines = [
    ...
    `description: ${JSON.stringify(toolMetaDescription(tool, permission))}`,
    '---',
    '',
...
function indexPage(tools, permissions) {
  ...
  return [
    '---',
    'title: MCP tools',
    `description: Browse all ${tools.length} RyTask MCP tools — the actions an AI agent can take over the Model Context Protocol, each with its required permission and JSON input schema.`,
    '---',
    '',
    '{/* Generated by apps/docs/scripts/generate-mcp-pages.mjs — do not edit by hand. */}',
    '',
