---
title: MCP parity
description: MCP parity in RyTask — AI agents are first-class clients with a tool for every capability, wired to the same services, and a CI gate fails when one is missing.
---
...
Today the registry stands at 54 tools covering 54 capabilities. When a new feature lands, its
...
them. Time tracking is covered — an agent can start, stop, and read a timer, log time after the
fact, and pull the time report — while a few narrower capabilities (editing or deleting past
entries, the interruption ledger, and the personal weekly summary) stay REST-only for now
rather than being silent gaps.
