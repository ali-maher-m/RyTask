---
title: MCP & AI agents
description: RyTask's first-party MCP server gives AI agents full parity with the UI — anything a person can do, an agent can do through 54 tools under the same permissions.
---
...
statuses, track time, search, comment, read notifications, and administer membership and
tokens.

## The parity promise

The MCP server is not a curated subset. RyTask's rule is **100% parity**: every service
capability the application exposes has a corresponding MCP tool. Today that is **54 tools**,
...
The full catalog — names, inputs, and outputs for all 54 tools — is in the
