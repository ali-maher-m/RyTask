...
```bash
git clone https://github.com/rytask/rytask.git
cd rytask
```
...
Other variables worth a look before you boot: `ALLOW_PUBLIC_SIGNUP` (default `false` —
invite-only), `ACCESS_TOKEN_TTL_SECONDS` (hard-capped at 900 seconds; larger values are
clamped), the `SLACK_*` variables if you use Slack capture (note that a configured Slack app
...
are public in the repository). The committed `migrate` command always runs the seed, and
today you should let it: self-registration only joins an existing organization, so a
completely empty database would leave you with no way to create the first account. A
first-run setup screen that replaces this is planned <StatusBadge status="coming-soon" />.
