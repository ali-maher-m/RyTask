...
- [ ] T001 Add additive runtime deps to `apps/api/package.json`: `@slack/web-api`, `@slack/oauth`
...
- [ ] T002 [P] Add Slack + MCP environment variables (no new compose service — Principle VII) to
...
- [ ] T003 [P] Create the Slack bounded-module directory skeleton under `apps/api/src/modules/slack/`
      (empty `controllers/`, `providers/`, `repositories/`, `processors/`, `domain/`, `guards/`,
      `events/`) per plan.md §Project Structure.
- [ ] T004 [P] Create the MCP transport-edge directory skeleton under `apps/api/src/mcp/`
...
- [ ] T005 Add `captureSourceEnum = pgEnum('capture_source', ['WEB','SLACK','MCP','API'])` to
      `packages/db/src/enums.ts` (data-model §1.3).
- [ ] T006 In `packages/db/src/tables.ts` add: (a) `work_items.source` column
...
- [ ] T007 Generate the Drizzle migration into `packages/db/migrations/` (`pnpm --filter @rytask/db
...
- [ ] T009 [P] Add `source: CaptureSource` to the `WorkItem` output DTO and thread an optional `source`
      into the create input plumbing in `packages/contracts/src/work-items.contract.ts`
      (capture-source.md §2).
- [ ] T010 [P] Create `packages/contracts/src/slack.contract.ts` with `SlackConnectionDto`,
...
- [ ] T011 [P] Create `packages/contracts/src/mcp/tool-io.ts` mapping each of the 49 registry tools to
...
- [ ] T012 Make `WorkItemsService.create` / `CreateWorkItemProvider` accept an optional `source`
...
- [ ] T013 [P] Add a `Crypto` port + AES-256-GCM adapter under `apps/api/src/common/crypto/` (bot-token
...
- [ ] T014 Extend the `SlackPort` interface in `apps/api/src/common/ports/slack.port.ts` with
...
- [ ] T015 Create the real Slack adapter `apps/api/src/common/adapters/slack/slack.adapter.ts`
...
- [ ] T016 Create `apps/api/src/modules/slack/slack.module.ts` and the public
...
- [ ] T017 [P] Implement `apps/api/src/modules/slack/repositories/slack-workspaces.repository.ts`
      extending `TenantScopedRepository` (auto-`WHERE organization_id`) (depends on T006, T016).
- [ ] T018 [P] Implement `apps/api/src/modules/slack/repositories/slack-users.repository.ts`
      extending `TenantScopedRepository` (depends on T006, T016).
- [ ] T019 [P] Tenancy-isolation specs proving no cross-tenant read/write for both tables, in
...
- [ ] T020 Create the Slack module test plan skeleton
...
- [ ] T021 Create `apps/api/src/mcp/mcp.module.ts` and `apps/api/src/mcp/mcp-server.factory.ts`
...
- [ ] T022 [P] Implement `apps/api/src/mcp/mcp-auth.ts`: resolve a PAT (`Authorization: Bearer` / stdio
      `RYTASK_PAT`) into a `Principal` via the **existing M0** token verification; update `lastUsedAt`
      (research D9).
- [ ] T023 [P] Implement `apps/api/src/mcp/mcp-session.ts`: transient per-session active-workspace
      (`McpSessionContext`), validated against `workspaces.list` (data-model §2.1).
- [ ] T024 [P] Implement `apps/api/src/mcp/mcp-errors.ts`: map domain exceptions to
      `INVALID_ARGUMENT` / `PERMISSION_DENIED` / `NOT_FOUND` (mcp-server.md §7, research D12).
- [ ] T025 Implement the `apps/api/src/mcp/tools/tool-dispatch.ts` scaffold: iterate registry tools,
      ...
      T022, T024, T011).
- [ ] T026 [P] Implement `apps/api/src/mcp/tools/pagination.ts`: wrap the existing keyset-paginated
      services into `{ items, nextCursor }` with opaque cursor + `fields` projection (research D14;
      depends on T011).
- [ ] T027 Create the MCP edge test plan `apps/api/src/mcp/mcp.testplan.ts` (declares the per-tool
...
- [ ] T031 [P] [US1] Integration test (real PG)
      ...
      (FR-SLK-001/002).
- [ ] T032 [P] [US1] Integration test (real PG)
...
- [ ] T033 [US1] Implement `apps/api/src/modules/slack/domain/slack-oauth-state.policy.ts` — pure
      HMAC-signed, short-TTL `state` nonce bound to `{organizationId, workspaceId, adminUserId, exp}`
      (research D16).
- [ ] T034 [US1] Implement `apps/api/src/modules/slack/providers/connect-slack.provider.ts`: exchange
      ...
      T013, T017, T018).
- [ ] T035 [US1] Implement `apps/api/src/modules/slack/providers/disconnect-slack.provider.ts`: revoke
      Slack token + set `revokedAt` (depends on T015, T017).
- [ ] T036 [US1] Implement `apps/api/src/modules/slack/providers/get-connection.provider.ts` returning
      `SlackConnectionDto` (status/team/connectedAt/defaultProjectId) (depends on T017).
- [ ] T037 [US1] Implement `apps/api/src/modules/slack/controllers/slack-oauth.controller.ts`:
...
- [ ] T038 [US1] Implement `apps/api/src/modules/slack/controllers/slack-admin.controller.ts`:
...
- [ ] T039 [US1] Append US1 required tests (T028–T032) to
      `apps/api/src/modules/slack/module.testplan.ts` so `check-required-tests` enforces them.

### Web for User Story 1

- [ ] T040 [P] [US1] Create the typed Slack admin client `apps/web/lib/api/slack.ts`
      (status/patch/disconnect/install) against `@rytask/contracts` (web-surfaces.md §1).
- [ ] T041 [P] [US1] Add the cosmetic `integrations:admin` capability (OWNER|ADMIN) to
      `apps/web/lib/auth/capabilities.ts` with its `reason(...)` copy (web-surfaces.md §2).
- [ ] T042 [US1] Build `apps/web/app/(app)/settings/integrations/page.tsx` +
...
- [ ] T043 [US1] Add the **Integrations** entry to the settings nav in the app shell, gated by `can()`
      (web-surfaces.md §1; depends on T042).
- [ ] T044 [P] [US1] Add `apps/web/e2e/connect-slack.e2e.spec.ts` (admin connects via stubbed OAuth →
...
- [ ] T046 [P] [US2] Contract test
      ...
      §1/§2).
- [ ] T047 [P] [US2] Integration test (real PG)
...
- [ ] T048 [US2] Implement the pure `apps/api/src/modules/slack/domain/slack-signature.policy.ts`
      (`v0=` HMAC-SHA256 over `v0:{ts}:{rawBody}`, `timingSafeEqual`, ≤300 s window) (research D4).
- [ ] T049 [US2] Implement `apps/api/src/modules/slack/guards/slack-signature.guard.ts` that reads the
      **raw body** and applies the policy before any handler work (research D4; depends on T048).
- [ ] T050 [US2] Configure the per-route **raw-body** parser for the Slack webhook paths only (in
      `apps/api/src/main.ts` / Slack module wiring) so signature bytes are exact and other JSON parsing is
      unaffected (research D4).
- [ ] T051 [US2] Implement the capture queue `apps/api/src/modules/slack/processors/slack-capture.queue.ts`
      (BullMQ queue + worker gated by `WORKER=1`) (research D7).
- [ ] T052 [US2] Implement `apps/api/src/modules/slack/processors/slack-capture.processor.ts`: dedupe via
...
- [ ] T053 [US2] Implement `apps/api/src/modules/slack/providers/capture-from-slack.provider.ts`:
...
- [ ] T054 [US2] Implement `apps/api/src/modules/slack/controllers/slack-events.controller.ts`
...
- [ ] T055 [US2] Post the confirmation via `SlackPort.respond(response_url, …)` from the processor — item
...
- [ ] T056 [US2] Append US2 required tests (T045–T047) to
...
- [ ] T057 [P] [US3] Unit test `apps/api/src/modules/slack/domain/slack-blocks.spec.ts`: modal +
      confirmation builders produce valid Block Kit and contain **no** tokens/secrets
      (slack-capture-flow.md §4).
- [ ] T058 [P] [US3] Integration test (real PG)
...
- [ ] T059 [P] [US3] Contract test (interactivity) added to
...
- [ ] T060 [US3] Implement the pure `apps/api/src/modules/slack/domain/slack-blocks.ts` — modal view +
      confirmation message builders (slack-capture-flow.md §4).
- [ ] T061 [US3] Implement `apps/api/src/modules/slack/providers/open-capture-modal.provider.ts`:
...
- [ ] T062 [US3] Extend `apps/api/src/modules/slack/controllers/slack-events.controller.ts`: the
...
- [ ] T063 [US3] Extend `apps/api/src/modules/slack/processors/slack-capture.processor.ts` to handle
      `kind: 'modal_submit'` → `WorkItemsService.create({ …selected, source:'SLACK', reporterId })` +
      confirmation (depends on T052, T053).
- [ ] T064 [US3] Append US3 required tests (T057–T059) to
      `apps/api/src/modules/slack/module.testplan.ts`.
...
- [ ] T065 [P] [US4] Integration test `apps/api/src/mcp/mcp-auth.int.spec.ts`: PAT → principal; revoked
      PAT denied mid-session; scope ∩ role enforced (FR-MCP-002, SC-004).
- [ ] T066 [P] [US4] Integration test (real PG) `apps/api/src/mcp/tools/mcp-capture.int.spec.ts`:
...
- [ ] T067 [P] [US4] Contract test `apps/api/src/mcp/tools/mcp-pagination.contract.spec.ts`:
      `list_issues` / `search` return cursored, filtered, field-selected results, **never truncated**
      (FR-MCP-005, US4.3).
- [ ] T068 [P] [US4] Tenant-isolation test `apps/api/src/mcp/mcp-tenant-isolation.spec.ts`: a foreign id
      yields `NOT_FOUND`/`PERMISSION_DENIED`; **0** cross-tenant rows (SC-004).
- [ ] T069 [P] [US4] Contract test `apps/api/src/mcp/tools/context-tools.contract.spec.ts`:
      `whoami`, `list_workspaces`, `get_workspace`, `set_active_workspace` return principal/scopes and
      scope subsequent calls (FR-MCP-003).
- [ ] T070 [P] [US4] Per-tool contract test `apps/api/src/mcp/tools/work-items-tools.contract.spec.ts`
...
- [ ] T071 [P] [US4] Per-tool contract test `apps/api/src/mcp/tools/projects-tools.contract.spec.ts`
...
- [ ] T072 [P] [US4] Per-tool contract test `apps/api/src/mcp/tools/statuses-tools.contract.spec.ts`
      covering the 5 statuses tools: `list_statuses, create_status, update_status, reorder_statuses,
      delete_status`.
- [ ] T073 [P] [US4] Per-tool contract test `apps/api/src/mcp/tools/views-tools.contract.spec.ts`
      covering the 4 views tools: `list_views, save_view, update_view, delete_view`.
- [ ] T074 [P] [US4] Per-tool contract test `apps/api/src/mcp/tools/collab-tools.contract.spec.ts`
      covering the 4 comments/notifications tools: `list_comments, add_comment, list_notifications,
      update_notification`.
- [ ] T075 [P] [US4] Per-tool contract test `apps/api/src/mcp/tools/search-tool.contract.spec.ts`
      covering `search` (ranked, permission-scoped).
- [ ] T076 [P] [US4] Per-tool contract test `apps/api/src/mcp/tools/org-tools.contract.spec.ts`
...
- [ ] T077 [P] [US4] Per-tool contract test `apps/api/src/mcp/tools/token-tools.contract.spec.ts`
      covering the 3 PAT tools: `list_api_tokens, create_api_token, revoke_api_token`.

### Implementation for User Story 4

- [ ] T078 [US4] Implement `apps/api/src/mcp/tools/context-tools.ts`
...
- [ ] T079 [US4] Wire the 14 work-items/labels tools in `apps/api/src/mcp/tools/tool-dispatch.ts` to the
      existing `WorkItemsService`/`LabelsService` (capture tools pass `source:'MCP'`) (depends on T025,
      T011, T026).
- [ ] T080 [US4] Wire the projects (7), statuses (5), and views (4) tools in `tool-dispatch.ts` to
      `ProjectsService`/`StatusesService`/`ViewsService` (depends on T025).
- [ ] T081 [US4] Wire the comments/notifications (4) and `search` (1) tools in `tool-dispatch.ts` to
      `CommentsService`/`NotificationsService`/`SearchService` (depends on T025, T026).
- [ ] T082 [US4] Wire the org-settings/membership (7) and PAT (3) tools in `tool-dispatch.ts` to
      `OrgsService`/membership/`IdentityService` (destructive tools honored) (depends on T025).
- [ ] T083 [US4] Implement the remote transport `apps/api/src/mcp/transport/mcp-http.controller.ts`
      (streamable HTTP/SSE at `POST/GET /mcp`, `Authorization: Bearer <PAT>`) mounted in the `api` process
      (research D10; depends on T021, T022).
- [ ] T084 [US4] Implement the local stdio entrypoint `apps/api/src/mcp/transport/mcp-stdio.entry.ts` +
      ...
      T022).
- [ ] T085 [US4] Append all MCP required tests (T065–T077) to `apps/api/src/mcp/mcp.testplan.ts` and run
...
- [ ] T086 [P] [US5] Integration test (real PG)
...
- [ ] T087 [P] [US5] Contract test added to
...
- [ ] T088 [US5] Implement `apps/api/src/modules/slack/providers/list-slack-users.provider.ts` returning
      `SlackUserMappingDto[]` (mapped + unmapped) (depends on T018).
- [ ] T089 [US5] Implement `apps/api/src/modules/slack/providers/map-slack-user.provider.ts` (link +
      unlink, idempotent) (depends on T018).
- [ ] T090 [US5] Extend `apps/api/src/modules/slack/controllers/slack-admin.controller.ts` with
...
- [ ] T091 [US5] Ensure the unmapped-captor "link your account" ephemeral prompt fires from the capture
      processor in `apps/api/src/modules/slack/processors/slack-capture.processor.ts` (US5.3; depends on
      T055).
- [ ] T092 [US5] Build `apps/web/app/(app)/settings/integrations/slack-users/page.tsx` +
...
- [ ] T093 [P] [US5] Add `apps/web/e2e/slack-users.e2e.spec.ts` (list, map, unmap; unmapped highlighted)
      and register it in `apps/web/web.testplan.ts`.
- [ ] T094 [US5] Append US5 server tests (T086–T087) to `apps/api/src/modules/slack/module.testplan.ts`.
...
- [ ] T095 [US6] Expose the MCP endpoint config to the web app (`MCP_PUBLIC_URL` → `httpUrl` + stdio
      hint) via the existing public-config path consumed by `apps/web/lib/` (data-model §3, quickstart §2).
- [ ] T096 [US6] Build `apps/web/app/(app)/settings/agent-access/page.tsx` + `agent-access-client.tsx`:
      ...
      T095).
- [ ] T097 [US6] Add the **Agent access** entry to the settings nav (visible to all — every user manages
      own PATs) (web-surfaces.md §1; depends on T096).
- [ ] T098 [P] [US6] Add `apps/web/e2e/agent-access.e2e.spec.ts` (endpoint + steps shown; create PAT
...
- [ ] T099 [P] [US7] Component test `apps/web/components/work-item/source-badge.spec.tsx`: renders
      Web/Slack/Agent/API with a **text label**, token-only styling (capture-source.md §3; register in
      `apps/web/web.testplan.ts`).

### Implementation for User Story 7

- [ ] T100 [P] [US7] Implement `apps/web/components/work-item/source-badge.tsx` — a `@rytask/ui` `Badge`
      using `var(--info-soft)`/`var(--info-fg)`, mapping `MCP → "Agent"` (research D17, web-surfaces.md
      §3.D).
- [ ] T101 [US7] Render `SourceBadge` on the work-item detail + list rows and inside the `CREATED`
      activity entry in `apps/web/` (consuming `work_items.source` already in the payload from T012)
      (FR-WEB-112; depends on T100).
- [ ] T102 [US7] Add `apps/web/e2e/source-badge.e2e.spec.ts` asserting items created via web/Slack/MCP
...
- [ ] T103 [P] [US8] Webhook integration test (real PG)
...
- [ ] T104 [P] [US8] Disconnect-interplay integration test
...
- [ ] T105 [P] [US8] Forged-tenant test
      `apps/api/src/modules/slack/processors/slack-capture.forged-team.spec.ts`: a foreign/forged
      `team_id` cannot write into another org (FR-X-001, data-model §5).
- [ ] T106 [P] [US8] MCP categorized-error contract test `apps/api/src/mcp/mcp-errors.contract.spec.ts`:
...
- [ ] T107 [US8] Add the ≤300 ms p95 server-side create assertion for cross-channel capture
      (FR-CAP-001/SC-002) — a perf assertion in the create integration path and/or a `k6` script under
      `infra/` (research D7, slack-capture-flow.md §6).
- [ ] T108 [US8] Append US8 required tests (T103–T106) to the relevant test plans
...
- [ ] T109 [P] Run `pnpm check:mcp-parity` and confirm it stays **green at 49/49** (transport now live; no
      tools added or orphaned — research D11; plan.md Complexity Tracking).
- [ ] T110 [P] Run `pnpm check:required-tests` and confirm the Slack module, MCP edge, and web test plans
      have **no missing** declared tests (Principle V).
- [ ] T111 [P] Run `pnpm check:design-tokens` and confirm the four new web surfaces are token-only (no raw
      hex/px, no gradient/blur/floaty-shadow/non-system-font/emoji-chrome) — Principle VIII, FR-X-003.
- [ ] T112 [P] Run `pnpm check:boundaries` (dependency-cruiser): Slack calls other modules **only** via
      their `*.contract.ts`; the MCP edge reaches into **no** module internals (Principle III).
- [ ] T113 [P] Confirm axe a11y on the connect-Slack and Agent-access journeys (keyboard, focus-trap,
      `role="alert"`, `prefers-reduced-motion`) (web-surfaces.md §4).
- [ ] T114 Update `README.md` / docs with the Slack + MCP env vars and the stdio `mcp:stdio` entrypoint
      (quickstart §2) — no new compose service (Principle VII).
- [ ] T115 Run the full `quickstart.md` §4 verification matrix (US1–US8) against the live stack and the
      `pnpm lint && pnpm typecheck && pnpm test && pnpm test:e2e` gates.
